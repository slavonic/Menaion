...
$FONTFORGE_BIN -lang=ff -c 'Open($1); Generate($2)' sources/Menaion.sfd sources/Menaion-Regular.ufo

# Dirty hack to add the openTypeOS2Selection key to the fontinfo.plist file
FONTINFO=`cat sources/Menaion-Regular.ufo/fontinfo.plist`
echo "${FONTINFO/    <key>openTypeOS2Type<\/key>/    <key>openTypeOS2Selection</key>$'\n'    <array>$'\n'      <integer>7</integer>$'\n'    </array>$'\n'    <key>openTypeOS2Type</key>}" > sources/Menaion-Regular.ufo/fontinfo.plist
